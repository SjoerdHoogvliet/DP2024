...
FROM medewerkers
WHERE functie = 'TRAINER' OR functie = 'VERKOPER'
GROUP BY mnr
HAVING gbdatum < CAST('1980-01-01' AS date)
...
FROM inschrijvingen
WHERE begindatum <= CAST('2019-12-31' AS date) AND begindatum >= CAST('2019-01-01' AS date)
...
SELECT DATE_PART('YEAR', AGE(NOW()::date, MIN(gbdatum)))-DATE_PART('YEAR', AGE(NOW()::date, MAX(gbdatum))) AS verschil,
       AVG(AGE(NOW()::date, gbdatum)) AS gemiddeld
FROM medewerkers
...
SELECT COUNT(*) AS aantal_medewerkers,
       ...
       AVG(comm) FILTER (WHERE functie = 'VERKOPER') AS commissie_verkopers
FROM medewerkers
